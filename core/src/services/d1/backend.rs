...
use crate::services::d1::delete::D1Deleter;
use crate::services::d1::writer::D1Writer;
...
impl D1Accessor {
    fn new(core: D1Core) -> Self {
        let info = AccessorInfo::default();
        info.set_scheme(Scheme::D1);
    ...
    async fn stat(&self, path: &str, _: OpStat) -> Result<RpStat> {
        ...
        if p == build_abs_path(&self.root, "") {
            ...
            match bs {
                ...
                None => Err(Error::new(ErrorKind::NotFound, "key not found in d1")),
            }
        }
    }
    ...
    async fn read(&self, path: &str, args: OpRead) -> Result<(RpRead, Self::Reader)> {
        ...
        let buffer = if range.is_full() {
            ...
            match self.core.get(&p).await? {
                Some(bs) => bs,
                None => return Err(Error::new(ErrorKind::NotFound, "key not found in d1")),
            }
        } else {
