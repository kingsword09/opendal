...
describe.runIf(operator.capability().write && operator.capability().stat)('sync statOptions tests', () => {
  const op = operator
  const capability = op.capability()

  test.runIf(capability.statWithIfMatch)('stat with ifMatch', () => {
  ...
  test.runIf(capability.statWithIfNoneMatch)('stat with ifNoneMatch', () => {
  ...
  test.runIf(capability.statWithIfModifiedSince)('stat with ifModifiedSince', () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
    ...
    setTimeout(() => {
      ...
      expect(() => op.statSync(filename, { ifModifiedSince: sinceAdd.toISOString() })).toThrowError('ConditionNotMatch')

      op.deleteSync(filename)
    }, 1000)
  })

  test.runIf(capability.statWithIfUnmodifiedSince)('stat with ifUnmodifiedSince', () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
  ...
  test.runIf(capability.statWithVersion)('stat with version', () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
  ...
  test.runIf(capability.statWithVersion)('stat with not existing version', () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
