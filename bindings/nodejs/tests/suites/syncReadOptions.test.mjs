...
describe.runIf(operator.capability().read && operator.capability().write)('sync read options', () => {
  const op = operator
  ...
  test.runIf(capability.readWithIfModifiedSince)('read with if modified since', () => {
    ...
    setTimeout(() => {
      ...
      expect(() => op.readSync(filename, { ifModifiedSince: sinceAdd.toISOString() })).toThrowError('ConditionNotMatch')
      op.deleteSync(filename)
    }, 1000)
  })
...
describe.runIf(operator.capability().read && operator.capability().write)('sync reader options', () => {
  const op = operator
  const capability = op.capability()
