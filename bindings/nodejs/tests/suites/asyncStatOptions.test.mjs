...
describe.runIf(operator.capability().write && operator.capability().stat)('async statOptions tests', () => {
  const op = operator
  const capability = operator.capability()

  test.runIf(capability.statWithIfMatch)('stat with ifMatch', async () => {
  ...
  test.runIf(capability.statWithIfNoneMatch)('stat with ifNoneMatch', async () => {
  ...
  test.runIf(capability.statWithIfModifiedSince)('stat with ifModifiedSince', async () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
  ...
  test.runIf(capability.statWithIfUnmodifiedSince)('stat with ifUnmodifiedSince', async () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
  ...
  test.runIf(capability.statWithVersion)('stat with version', async () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
  ...
  test.runIf(capability.statWithVersion)('stat with not existing version', async () => {
    const size = 5 * 1024 * 1024
    const filename = `random_file_${randomUUID()}`
    const content = generateFixedBytes(size)
